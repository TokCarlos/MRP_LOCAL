...
while ($true) {
    ...
    $hc = $LASTEXITCODE
    if ($hc -eq 0) {
        Add-MrpLogLine -LogFile $log -Origem "watchdog" -Status "OK" -Detalhe "Healthcheck OK"
        if ($Once) { exit 0 }
    } else {
        $now = Get-Date
        for ($i = $restartTimes.Count - 1; $i -ge 0; $i--) {
            if (($now - $restartTimes[$i]).TotalMinutes -gt 60) { $restartTimes.RemoveAt($i) }
        }
        if ($restartTimes.Count -ge $maxRestarts) {
            Add-MrpLogLine -LogFile $log -Origem "watchdog" -Status "COOLDOWN" -Detalhe "Limite de $maxRestarts restarts/hora atingido. Aguardando $cooldown segundos."
            Start-Sleep -Seconds $cooldown
        } else {
            Add-MrpLogLine -LogFile $log -Origem "watchdog" -Status "RESTART" -Detalhe "Healthcheck falhou. Tentando iniciar."
            & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $start | Out-Null
            $restartTimes.Add((Get-Date))
            if ($Once) { Start-Sleep -Seconds 3 }
        }
        if ($Once) {
            & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $health | Out-Null
            exit $LASTEXITCODE
